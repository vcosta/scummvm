...
int MidiDriver_AmigaMac::open() {
	...
	Common::File file;
	if (file.open("Sound.p")) {
		debug("Found SCI0 instruments file!");
		if (!loadInstrumentsSCI0(file))
			return Common::kUnknownError;
		debug("Loaded SCI0 instruments file!");
		/*
		_isSci1 = true;
		_isSci1Early = true;
		if (!loadInstrumentsSCI1(file))
			return Common::kUnknownError;*/
		file.close();
	} else {
		debug("Couldn't open SCI instruments file!");
	}
